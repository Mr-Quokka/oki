...
if ! pkg-config nlohmann_json --exists || ! pkg-config libcurl --exists || ! pkg-config minizip --exists || ! pkg-config libssl --exists || ! command -v gcovr &> /dev/null || ! [ -f /usr/include/doctest/doctest.h ]; then
  apt-get install -y nlohmann-json3-dev libcurl4-openssl-dev libminizip-dev libssl-dev doctest-dev gcovr
fi
./configure.sh -d
make $*
EOF
